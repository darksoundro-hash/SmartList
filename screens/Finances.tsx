...
import { getFinancialInsight } from '../SmartList/services/gemini';
import { supabase } from '../SmartList/services/src/lib/supabase';

const Finances: React.FC = () => {
   const [history, setHistory] = useState<HistoryEntry[]>([]);
   const [financialInsight, setFinancialInsight] = useState('Analisando seus hábitos de consumo...');
   ...
   const fetchHistoryAndInsights = async () => {
      try {
         ...
         // Buscar insight da IA baseado nos dados reais
         if (mappedHistory.length > 0) {
            const total = mappedHistory.reduce((acc: number, curr: HistoryEntry) => acc + curr.totalValue, 0);
            const items = mappedHistory.reduce((acc: number, curr: HistoryEntry) => acc + curr.itemCount, 0);

            // Only call AI if we have data to analyze
            const res = await getFinancialInsight(total, items);
            setFinancialInsight(res);
         } else {
            setFinancialInsight("Comece a finalizar suas listas para que a IA possa analisar sua saúde financeira!");
         }
         ...
         <main className="flex-1 flex flex-col h-screen overflow-y-auto relative">
            ...
            <div className="p-6 lg:p-10 space-y-8 pb-32 max-w-7xl mx-auto w-full">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div className="bg-primary p-6 rounded-[2rem] flex flex-col justify-between shadow-2xl shadow-primary/20 group hover:scale-[1.02] transition-all cursor-pointer">
                     <div className="flex justify-between items-start text-background-dark">
                        <span className="text-[10px] font-black uppercase tracking-widest">Economia IA</span>
                        <div className="size-10 rounded-xl bg-black/10 flex items-center justify-center"><Sparkles size={20} /></div>
                     </div>
                     <div className="mt-4 text-background-dark">
                        <h3 className="text-2xl font-black">R$ 142,50</h3>
                        <p className="text-[10px] font-black uppercase mt-1 tracking-widest opacity-60">Poupados este mês</p>
                     </div>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 bg-surface-dark rounded-[2.5rem] border border-border-green p-8 shadow-2xl">
                  ...
                  {/* Insight da IA */}
                  <div className="bg-gradient-to-br from-[#112217] to-background-dark rounded-[2.5rem] border border-border-green p-8 flex flex-col gap-6 relative overflow-hidden shadow-2xl">
                     <div className="absolute -right-20 -bottom-20 size-64 bg-primary/5 rounded-full blur-3xl pointer-events-none"></div>
                     <div className="flex items-center gap-4 z-10">
                        <div className="size-14 rounded-2xl bg-primary/20 flex items-center justify-center text-primary shadow-lg"><Sparkles size={28} /></div>
                        <h3 className="text-white font-black text-lg uppercase tracking-tight">AI Advisor</h3>
                     </div>
                     <div className="flex-1 flex flex-col justify-center gap-4 z-10">
                        <p className="text-white text-base font-medium leading-relaxed italic border-l-2 border-primary pl-6">
                           {isLoadingInsight ? "Sincronizando dados com Gemini..." : financialInsight}
                        </p>
                        <p className="text-[10px] text-text-secondary font-black uppercase tracking-widest mt-4">Baseado no seu histórico recente</p>
                     </div>
                     <button className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center gap-3 font-bold hover:bg-white/10 transition-all z-10 active:scale-95">
                        Ver Relatório Completo <ChevronRight size={18} />
                     </button>
                  </div>
               </div>

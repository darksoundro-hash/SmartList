...
const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#0b1810] selection:bg-primary selection:text-background-dark overflow-hidden">
      {/* Lado Esquerdo - Branding e Benefícios */}
      {/* ... (mantendo o lado esquerdo igual para economizar tokens neste diff, se possível, mas preciso garantir que não quebre) ... */}
      {/* Vou substituir o componente inteiro para garantir integridade, mas reutilizando o JSX existente onde possível */}
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12 lg:p-24 relative">
        <div className="w-full max-w-md flex flex-col gap-10 animate-in fade-in slide-in-from-right-8 duration-700">
          ...
          <div className="self-center lg:self-end bg-white/5 border border-white/10 p-1.5 rounded-full flex w-fit mb-4">
            <button
              onClick={() => setIsLogin(true)}
              className={`px-8 py-2.5 rounded-full font-black text-xs uppercase tracking-widest transition-all ${isLogin ? 'bg-primary text-background-dark shadow-xl' : 'text-text-muted hover:text-white'}`}
            >
              Login
            </button>
            <button
          ...
          <div className="flex flex-col gap-6">
            <button
              ...
              disabled={isLoading}
              className="flex w-full items-center justify-center gap-3 bg-white text-background-dark font-black h-14 rounded-full hover:bg-gray-100 transition-all active:scale-95 disabled:opacity-50"
            >
            ...
            <form className="flex flex-col gap-5" onSubmit={handleAuth}>
              ...
              {!isLogin && (
                <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  <div className="relative flex items-center group">
                    <User className="absolute left-5 text-text-muted group-focus-within:text-primary transition-colors" size={20} />
                    <input
                      className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 text-white focus:border-primary/50 focus:ring-4 focus:ring-primary/5 focus:outline-none transition-all placeholder:text-white/10"
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="relative flex items-center group">
                  <Mail className="absolute left-5 text-text-muted group-focus-within:text-primary transition-colors" size={20} />
                  <input
                    className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 text-white focus:border-primary/50 focus:ring-4 focus:ring-primary/5 focus:outline-none transition-all placeholder:text-white/10"
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="relative flex items-center group">
                  <Lock className="absolute left-5 text-text-muted group-focus-within:text-primary transition-colors" size={20} />
                  <input
                    className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 text-white focus:border-primary/50 focus:ring-4 focus:ring-primary/5 focus:outline-none transition-all placeholder:text-white/10"
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-primary text-background-dark h-16 rounded-full font-black text-lg hover:bg-[#0fdc53] hover:shadow-[0_0_30px_rgba(19,236,91,0.3)] transition-all transform active:scale-[0.97] mt-4 flex items-center justify-center gap-3 group relative overflow-hidden"
              >
                {isLoading ? (
